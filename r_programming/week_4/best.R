best <- function(state, outcome) {
    ...
    if(outcome == "heart attack") {
        winner <- state_results[which(state_results$heart_attack == min(state_results$heart_attack, na.rm = TRUE)),]
        return(winner$hospital)
    }
    if(outcome == "heart failure") {
        winner <- state_results[which(state_results$heart_failure == min(state_results$heart_failure, na.rm = TRUE)),]
        return(winner$hospital)
    }
    if(outcome == "pneumonia") {
        winner <- state_results[which(state_results$pneumonia == min(state_results$pneumonia, na.rm = TRUE)),]
        return(winner$hospital)
    }
}